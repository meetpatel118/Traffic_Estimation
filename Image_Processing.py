...
src_path ='E:\\Acadia\\Traffic Density\\Data\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2019\\02\\11\\'
dst_path ='E:\\Acadia\\Traffic Density\\Data\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2019\\02\\FG 1.0\\'
cropped_path = 'E:\\Acadia\\Traffic Density\\Data\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2019\\02\\Cropped 3.0\\'
masked_path = 'E:\\Acadia\\Traffic Density\\Data\\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2019\\02\\Masked 3.0\\'
contour_path = 'E:\\Acadia\\Traffic Density\\Data\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2019\\02\\Contours 3.0\\'
rename_path = 'E:\\Acadia\\Traffic Density\\Data\\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2019\\02\\Renamed\\'
path_of_day = 'E:\\Acadia\Traffic Density\\Data\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2019\\02\\'

time_info_file = 'timeinfo_file_20190211.csv'
prediction_data_dile = 'prediction_data_20190211.csv'
    ...
    with open(path_of_day + time_info_file, mode='w') as time_file:
        ...
        for file in os.listdir():
            ...
            for file in os.listdir():
                ...
                frame = 'mackaybridge_20190211-'+str(number).zfill(5)+'.jpg'
                ...
                img = cv.imread(file)
                if img is not None:
                    cv.imwrite(os.path.join(rename_path, 'mackaybridge_20190211-'+str(number).zfill(5)+'.jpg'), img)
            
                number+=1
    time_file.close()
    #close('E:\\Acadia\\Traffic Density\\Data\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2018\\12\\timeinfo_file_20181219.csv')
...
#             # cv.imshow('',crp_img)
#             #path = 'E:\\Acadia\\AI\\LML - Thesis\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2018\\12\\12\\Cropped\\'
#             cv.imwrite(os.path.join(dst_path , 'mackaybridge_20190211-'+str(number).zfill(5)+'.jpg'), cropped)
#             number+=1
##---------------------------------------------------------------------------------------------------------------------------------------------------
def masking_of_images():

    number = 0
    capture = cv.VideoCapture('E:\\Acadia\\Traffic Density\\Data\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2018\\12\\Renamed\\mackaybridge_20190211-%05d.jpg')
    ...
    while True:
        ...
        rect = cv.boundingRect(points) # returns (x,y,w,h) of the rect
        cropped = res[rect[1]: rect[1] + rect[3], rect[0]: rect[0] + rect[2]]
        cv.imwrite(os.path.join(cropped_path , 'mackaybridge_20190211-'+str(number).zfill(5)+'.jpg'), cropped)
        ...
        #cv.imshow('MOG 2', fgMask1)
        #cv.imshow('MOG 3', fgMask2)
        cv.imwrite(os.path.join(masked_path, 'mackaybridge_20190211-'+str(number).zfill(5)+'.jpg'), masked)
        number += 1
    return True
#*********************************************************************
    #cv.imshow('MOG2 Remove Shadow', fgMask1)
    #fgMask1[fgMask1==127]=0
    #cv.imshow('MOG2', fgMask1)
    #frame = cv.flip(frame, 1)
    #cv.imshow('Video Life2Coding', frame)

#----------------------------------------Connect nearby contours--------------------------------------
def connect_contours():
    number = 0
    os.chdir(masked_path)
    
    for file in os.listdir():
    
        img = cv.imread(os.path.join(masked_path, file))
        #img = np.array(Image.fromarray(masked).convert("RGB")) #use masked instead of loading from folder
        ...
        #cropped = cv.imread('E:\\Acadia\\Traffic Density\\Data\\Weather Code\\Webcam Data\\mackaybridge_20190211\\2018\\12\\Cropped\\'+file)
        ...
        cv.imwrite(os.path.join(contour_path, 'mackaybridge_20190211-cnt-'+str(number).zfill(5)+'.jpg'), Contour)
        number += 1
...
def multiply_image():
    ...
    for file in os.listdir():
        ...
        Contour = cv.imread(os.path.join(contour_path, file))
        ...
        cv.imwrite(os.path.join(dst_path , 'mackaybridge_20190211-fg-'+str(number).zfill(5)+'.jpg'), falcon)
